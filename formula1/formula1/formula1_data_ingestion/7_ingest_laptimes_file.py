...
laptimes_schema = StructType(fields = [StructField("raceId", IntegerType(), False), #nullable = False
...
laptimes_df = spark.read.schema(laptimes_schema).csv(f"{raw_folder_path}/{v_file_date}/lap_times.csv")
display(laptimes_df)
...
laptimes_df = laptimes_df.dropDuplicates(['raceId', 'driverId', 'lap', 'position', 'time', 'milliseconds'])
...
renamed_laptimes_df = laptimes_df.withColumnRenamed('raceId', 'race_id')\
...
renamed_laptimes_df = add_ingestion_time(renamed_laptimes_df) 
...
# overwrite_partition(renamed_laptimes_df, 'f1_processed', 'lap_times', 'race_id')
...
# renamed_laptimes_df = re_arrange_partition_column(renamed_laptimes_df, 'race_id')

# # Set partition overwrite mode to dynamic
# spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic")

# # Determine the path for the partitioned data
# partition_path = f"{processed_folder_path}/lap_times"
...
#     new_data_df = renamed_laptimes_df#.dropDuplicates(['race_id'])
...
#     renamed_laptimes_df.write.mode("overwrite").partitionBy('race_id').parquet(partition_path)
...
merge_delta_data(renamed_laptimes_df, 'f1_processed', 'lap_times', processed_folder_path, merge_condition, 'race_id')
...
delta_partition_path = f"{processed_folder_path}/lap_times"
...
if DeltaTable.isDeltaTable(spark, delta_partition_path):
    ...
    new_data_df = renamed_laptimes_df
    ...
    renamed_laptimes_df.write.mode("overwrite").partitionBy('race_id').format("delta").save(delta_partition_path)
...
results_after_df = spark.read.format("delta").load(f"{processed_folder_path}/lap_times")
